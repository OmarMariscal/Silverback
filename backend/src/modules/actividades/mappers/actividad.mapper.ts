...
> {
  ...
  public toDomain(raw: PrismaActividadPayload): ActividadEntity {
    ...
    return new ActividadEntity(
      ...
      raw.folio,
      raw.titulo,
      raw.justificacion || '', //P{risma puede devolver un nulo
      raw.objetivo_general || '',
      raw.objetivos_part || '',
      raw.meta_proyecto || '',
      raw.indicadores || '',
      raw.fecha_inicio || new Date(), // Manejo seguro si la fecha aún no se define
      raw.fecha_termino || new Date(),
      raw.es_rezago,
      auditoresIdsLimpio,

...
export function TarjetaActividadPOA({
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border overflow-hidden mb-6 transition-all ${colorBordeTarjeta}`}>
      ...
      <div onClick={handleToggle} className="p-6 border-b border-slate-100 flex justify-between items-start bg-white cursor-pointer hover:bg-slate-50 transition-colors">
        <div className="flex items-start space-x-5">
          ...
          <div className="pt-1">
            <h3 className="text-xl font-bold text-slate-800 leading-tight flex items-center">
              {titulo}
              {esRezagada && (
                <span className="ml-3 text-[10px] uppercase tracking-wider bg-red-100 text-red-600 px-3 py-1 rounded font-bold">Rezago Automático</span>
              )}
            </h3>
            ...
            <div className="flex flex-wrap items-center mt-3 gap-y-2 gap-x-6">
              ...
              <span className="flex items-center text-sm font-medium text-slate-600">
                <span className="text-slate-400 mr-1">Auditores:</span>
                {auditoresResumen && String(auditoresResumen).trim().length > 0 ? (
                  auditoresResumen
                ) : (
                  <span className="flex items-center text-sm font-medium text-slate-600">
                    Sin auditores registrados
                  </span>
                )}
              </span>
            </div>
        ...
        <div className="flex items-center space-x-2 pl-4">
          ...
          <button 
            ...
            onClick={(e) => { 
              e.stopPropagation(); 
              if (onConfigurarFichaTecnica) onConfigurarFichaTecnica(); 
            }}
            ...
            <>
              ...
              <div>
                <div className="flex justify-between items-end mb-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sub-actividades Específicas</h4>
                  <button 
                    type="button"
                    onClick={() => onAbrirModalSubactividades && onAbrirModalSubactividades(idActividad, titulo)}
                    className={`text-sm font-bold flex items-center px-4 py-2 rounded-lg transition-colors border cursor-pointer ${esRezagada ? 'bg-red-100 text-red-600 hover:bg-red-100 border-red-200/60' : 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border-indigo-100'}`}
                  >
                      ...
                      {subactividades && subactividades.length > 0 ? (
                        subactividades.map((sub) => (
                          <tr key={sub.id} className="hover:bg-slate-50/80 transition-colors group">
                            <td className="px-6 py-5 text-center"><span className="inline-block bg-slate-100 text-slate-600 font-bold text-xs px-2.5 py-1 rounded-md">{sub.folioSecuencial}</span></td>
                            <td className="px-6 py-5 text-slate-700 font-medium leading-relaxed pr-8">{sub.descripcion}</td>
                            <td className="px-6 py-5">
                              <span className="text-slate-600 font-medium flex items-center bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 whitespace-nowrap">
                                ...
                                {formatearFechaEspanol(sub.fechaInicioFormateada)} - {formatearFechaEspanol(sub.fechaTerminoFormateada)}
                              </span>
                            </td>
                            <td className="px-6 py-5 text-center">
                              <span className={`inline-flex items-center px-3 py-1.5 border rounded-lg text-xs font-bold ${esRezagada ? 'bg-red-100 text-red-600 hover:bg-rose-100 border-red-200/60' : 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border-indigo-100'}`}>
                        ...
                        <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-400 italic">No hay subactividades registradas.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
